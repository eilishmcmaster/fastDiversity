...
#' 
#' @description
#' This function calculates pi, Watterson's theta, and Tajima's D
...
#' @param n Number of individuals to use for calculations per site. If left as NULL, 
...
#' @param allele_count_min Minimum minor allele count to keep a locus, can be used to 
#'   exclude singletons.
...
#'   \item{pi}{Nucleotide diversity (raw pi).}
#'   \item{S}{Number of segregating sites.}
#'   \item{theta}{Watterson's theta.}
#'   \item{D}{Tajima's D statistic.}
#'   \item{real_missingness}{Proportion of missing data in the filtered genotype matrix.}
...
pi_theta_d_stats <- function(gt, genetic_group_variable, site_variable,
  ...
  for (group in genetic_groups) {
    ...
    if (length(sites) == 0) {
      message(sprintf("Skipping group '%s': no sites meet n >= %d.", group, minimum_n))
      next
    }
    ...
    if (maf > 0) {
      ...
      gt_group_missing <- gt_group_missing[, ..passing_maf_loci]
      if (ncol(gt_group_missing) == 0) next
    }
    
    if (allele_count_min > 0) {
      loci_min_ac      <- get_minor_allele_counts(gt_group_missing)
      passing_ac_loci  <- which(loci_min_ac >= allele_count_min)
      gt_group_missing <- gt_group_missing[, ..passing_ac_loci]
      if (ncol(gt_group_missing) == 0) next
    }
    ...
    for (s in seq_along(sites)) {
      ...
      if (length(which(site_variable_group == site)) <= 1) next
      
      gt_site <- gt_group_missing[which(site_variable_group == site), ]
      ...
      site_results <- c(
        ...
        "pi"               = as.numeric(td_out$raw_pi),
        "S"                = as.numeric(td_out$num_sites),
        "theta"            = as.numeric(td_out$watterson_theta),
        "D"                = as.numeric(td_out$tajima_d),
        "real_missingness" = sum(is.na(gt_site_subset2)) /
          (ncol(gt_site_subset2) * nrow(gt_site_subset2))
      )
